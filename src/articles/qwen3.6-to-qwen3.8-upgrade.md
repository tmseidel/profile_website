...
## 6. The Configuration Is Open Source

Everything in this article is reproducible from public sources. The full Ansible configuration — the `vllm` role, the group vars holding every knob mentioned above (`vllm_max_num_seqs`, `vllm_max_model_len`, `gpu_memory_utilization`, `vllm_speculative_config`, the Blackwell workarounds), and the model-switch history with git tags — is published under the MIT license at [github.com/tmseidel/local-llm-setup](https://github.com/tmseidel/local-llm-setup). It targets exactly this hardware (2× RTX 5060 Ti 16 GB on Ubuntu 24.04), runs one inference engine at a time (vLLM or Ollama) controlled by a single `inference_engine` variable so the active engine gets the full 32 GB VRAM, and includes systemd watchdogs that restart vLLM on failure. The latest release pins the configuration to the Qwen3.8-27B-W4A16-AWQ setup described in this article; earlier tags cover the previous generations (Qwen3.6-35B-A3B-AWQ-4bit, Qwen3-32B-AWQ), so the whole upgrade path is auditable in the git history.
...
6. [tmseidel/local-llm-setup — GitHub](https://github.com/tmseidel/local-llm-setup) — the complete open-source Ansible configuration (MIT license): the vLLM role, group vars, model-switching git history and releases, Blackwell workarounds, systemd watchdogs.
